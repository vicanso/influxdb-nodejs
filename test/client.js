...
describe('influxdb-nodejs:singleton', () => {
  ...
  it('write point success', done => {
    ...
    client.writePoint(measurement, fields, tags).then(data => {
      // data -> undefined
      assert.equal(data, undefined);
      return client.query(measurement).tag({
        uuid: uuid
      }).end();
    }).then(data => {
  ...
  it('get points success', done => {
    client.query(measurement)
      ...
      .then(data => {
        ...
        const keys = ["time", "bytes", "code", "size", "status",
          "use", "uuid", "value"
        ].sort();
  ...
  it('offset points success', done => {
    ...
    client.query(measurement)
      ...
      .then(data => {
        assert.equal(prevData.series[0].values[1].sort().join(','),
          data.series[0].values[0].sort().join(','));
        done();
      }).catch(done);
  });
  ...
  it('mean result success', done => {
    client.query(measurement)
      ...
      .then(data => {
        ...
        assert.equal(data.series[0].columns.sort().join(','),
          'mean,time');
