...
class Client {
  ...
  createRetentionPolicy(name, duration, replication, isDefault) {
    const internalData = internal(this);
    const database = internalData.opts.database;
    const q = InfluxQL.createRP(name, database, duration, replication || 1, isDefault || false);
  ...
  dropRetentionPolicy(name) {
    const internalData = internal(this);
    const database = internalData.opts.database;
    const q = InfluxQL.dropRetentionPolicy(name, database);
    return internalData.influx.query(q).then(_.noop);
  }
