...
class Client {
   ...
   * @param  {Object} schema      - the schema definition, if is undefined, will return the schme for the measurement
  ...
  schema(measurement, definition, options) {
    if (!definition) {
      return schema.get(measurement);
    }
    return schema.set(measurement, definition, options);
  }
}
