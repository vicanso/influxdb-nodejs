...
class Client {
	constructor(uri, options) {
		...
		if (_.isString(uri)) {
			...
			if (urlInfo.pathname) {
				tmpOptions.database = urlInfo.pathname.substring(1);
			}
			if (urlInfo.auth) {
				const authInfos = urlInfo.auth.split(':');
	...
	get availableServers() {
		return http.getAvailableServers();
	}

	/**
	 * [disabledServers description]
	 * @return {[type]} [description]
	 */
	get disabledServers() {
		return http.getDisabledServers();
	}
