...
class Client extends EventEmitter {
   ...
   * @param  {Object} options - [optional] The options for influxdb client, {loadBalancingAlgorithms: "round-robin"}.
   * loadBalancingAlgorithms can be url, leastconn, round-robin, first, url-path, default is round-robin
   */
  constructor(uri, options) {
    ...
    super();
    const opts = _.extend({
      loadBalancingAlgorithms: 'round-robin',
    }, options);
  ...
  startHealthCheck(ping) {
    ...
    internal(this).influx.startHealthCheck(ping || defaultPing);
    return this;
  }
  ...
  get unavailableServers() {
    ...
    const servers = internal(this).opts.servers;
    const availableServers = this.availableServers;
    const arr = [];
