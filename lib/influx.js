...
function getPostData(measurement, tags, values) {
	const time = values.time;
	delete values.time;
	return `${measurement},${format(tags)} ${format(values)} ${time}`;
}
...
class Influx {
	...
	write(data) {
		...
		const postData = getPostData(data.measurement, data.tags, data.values);
	...
	writeMeasurement(data) {
		...
		const url = `/write?${qs.stringify(queryData)}`;
		const postData = [];
		_.forEach(data, (v, measurement) => {
			_.forEach(v, tmp => {
				postData.push(getPostData(measurement, tmp.tags, tmp.values));
			});
		});
