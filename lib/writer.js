...
class Writer {
   ...
   * @param  {Set} queueSet - The query queue
   * @return {Writer}
   * @since 2.2.0
   */
  ...
  toJSON() {
    ...
    const data = {};
    _.forEach('measurement tags fields time precision'.split(' '), (key) => {
      if (!_.isUndefined(internalData[key])) {
        if (key === 'fields') {
          data[key] = schema.validate(data.measurement, internalData[key]);
          return;
        }
        data[key] = internalData[key];
      }
    });
  ...
  queue() {
    const internalData = internal(this);
    const queueSet = internalData.queueSet;
    /* istanbul ignore if */
    if (!queueSet) {
